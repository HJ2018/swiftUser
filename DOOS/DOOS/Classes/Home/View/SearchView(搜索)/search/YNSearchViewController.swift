...
open class YNSearchViewController: UIViewController, UITextFieldDelegate {
    ...
    override open func viewDidLayoutSubviews() {
        ...
        self.ynSearchTextfieldView.frame = CGRect(x: 20, y: kStatusBarHeight + 15, width: width - 40, height: 50)
        self.ynSearchView.frame = CGRect(x: 0, y: 60 + safeAreaTopInset, width: width, height: height - 60 - safeAreaTopInset)
    }

    open func ynSearchinit() {
        self.ynSearchTextfieldView = YNSearchTextFieldView(frame: CGRect(x: 50, y: 0, width: width - 100, height: 36))
        ...
        self.ynSearchView = YNSearchView(frame: CGRect(x: 0, y: 60, width: width, height: height - 60))
