...
open class YNSearchMainView: UIView {
    ...
    open func initView(categories: [String]) {
        self.categoryLabel = UILabel(frame: CGRect(x: margin, y: 0, width: kScreenWidth - 40, height: 50))
        self.categoryLabel.text = "热门搜索"
        ...
        for i in 0..<categories.count {
            ...
            let button = YNCategoryButton(frame: CGRect(x: formerWidth, y: formerHeight, width: size.width + 10, height: size.height + 15))
        ...
        self.searchHistoryLabel = UILabel(frame: CGRect(x: margin, y: originY + 30, width: kScreenWidth - 40, height: 40))
        self.searchHistoryLabel.text = "搜索历史"
    ...
    open func redrawSearchHistoryButtons() {
        ...
        for i in 0..<histories.count {
            let view = YNSearchHistoryView(frame: CGRect(x: margin, y: searchHistoryLabelOriginY + CGFloat(i * 40) , width: kScreenWidth - (margin * 2), height: 40))
...
//        self.clearHistoryButton.setTitle("清除搜索历史", for: .normal)
