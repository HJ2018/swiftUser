    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView.tag == 10 {
            if searchDataArray[indexPath.row] is AMapTip {
                ...
                if self.searchMapTip != nil{
                     self.searchMapTip!((searchDataArray[indexPath.row] as? AMapTip)!)
                }
            }else{
                
                if self.searchMapTip != nil{
                     self.searchMapPOI!((searchDataArray[indexPath.row] as? AMapPOI)!)
                }
            }
             
        }else{
             if self.searchMapPOI != nil{
                 self.searchMapPOI!((dataArray[indexPath.row] as? AMapPOI)!)
            }
        }
